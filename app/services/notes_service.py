...
class NoteService:
    async def get_all_notes(self, user_uid: uuid.UUID, session: AsyncSession) -> Any:
        statement = (
            select(Note).where(Note.to_user_uid == user_uid).order_by(desc(Note.created_at))
        )  # TODO filter by user
        result = await session.execute(statement)
        return result.scalars().all()

    async def get_note(self, note_uid: uuid.UUID, user_uid: uuid.UUID, session: AsyncSession) -> Any:
